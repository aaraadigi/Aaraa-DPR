...
export const PettyCash: React.FC<PettyCashProps> = ({ role, userId, userName }) => {
  ...
  const filteredEntries = useMemo(() => {
    // Finance sees all, PM sees all project related, SE sees only theirs
    ...
    // ... (rest of filtering logic from previous version)
            ...
            {pendingApprovals.map(entry => (
              <GlassCard key={entry.id} className="border-l-4 border-amber-500 !p-5 relative">
                <div className="flex justify-between items-start mb-3">
                  <div className="text-[10px] font-black bg-amber-50 text-amber-700 px-2 py-1 rounded uppercase">Finance Initiated</div>
                  <span className="text-xs font-black text-slate-900">₹{entry.amount.toLocaleString()}</span>
                </div>
                <h4 className="font-bold text-slate-800 text-sm mb-1">{entry.paid_to}</h4>
                <p className="text-[10px] text-slate-500 mb-3 line-clamp-2">{entry.remarks}</p>
                <div className="flex gap-2 border-t pt-3 mt-auto">
                  ...
                  <button onClick={() => handlePMAction(entry.id, 'Rejected')} className="flex-1 bg-white border border-red-200 text-red-600 py-2 rounded-lg text-[10px] font-bold flex items-center justify-center gap-1 hover:bg-red-50">
        ...
        {showForm && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-10">
            <GlassCard className="!p-8 border-2 border-indigo-100 shadow-2xl relative">
              <button onClick={() => setShowForm(false)} className="absolute top-4 right-4 p-2 text-slate-400 hover:text-slate-900"><X size={20}/></button>
              <h3 className="text-xl font-bold text-slate-800 mb-8 border-b border-slate-100 pb-4 text-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-6">
                  {isFinance && (
                    <div>
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Select Project for PM Approval</label>
                      <select 
                        value={selectedProject} 
                        onChange={e => setSelectedProject(e.target.value)}
                        className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl text-sm font-bold focus:border-indigo-500 outline-none"
                      >
                  ...
                  <div>
                    ...
                    <input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0.00" className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl text-2xl font-black text-indigo-600 outline-none" />
                  </div>
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Paid To</label>
                    <input type="text" value={paidTo} onChange={e => setPaidTo(e.target.value)} placeholder="Vendor Name" className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl text-sm font-bold outline-none" />
                  </div>
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Remarks</label>
                    <textarea value={remarks} onChange={e => setRemarks(e.target.value)} placeholder="Purpose of payment..." className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl text-sm outline-none" rows={3} />
                  </div>
                </div>
                <div className="space-y-6">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block text-center">Receipt Upload</label>
                  <label className={`w-full h-72 border-2 border-dashed rounded-3xl flex flex-col items-center justify-center cursor-pointer overflow-hidden transition-all bg-slate-50 ${screenshot ? 'border-green-500' : 'border-slate-200'}`}>
                    {screenshot ? <img src={screenshot} className="w-full h-full object-cover" /> : (
                      <div className="text-center group">
                        ...
                        <p className="text-xs font-bold text-slate-400">Upload Receipt</p>
                      </div>
                    )}
                    <input type="file" className="hidden" accept="image/*" onChange={handleCapture} />
                  </label>
                  <button onClick={handleSubmit} disabled={submitting} className="w-full bg-slate-900 text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-indigo-600 disabled:opacity-50">
        ...
        <div className="p-6 border-b flex flex-wrap items-center justify-between gap-4">
          <div className="flex bg-slate-100 p-1 rounded-xl">
            {['all', 'day', 'week', 'month'].map(f => (
              <button key={f} onClick={() => setFilterType(f as any)} className={`px-4 py-1.5 text-[10px] font-black uppercase rounded-lg transition-all ${filterType === f ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}>{f}</button>
            ))}
          </div>
          <div className="relative w-full max-w-xs">
            <Search className="absolute left-3 top-2.5 text-slate-300" size={16} />
            <input type="text" placeholder="Search entries..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-100 rounded-xl text-sm outline-none" />
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {filteredEntries.map(e => (
                <tr key={e.id} className="border-b hover:bg-indigo-50/20 transition-colors">
                  <td className="px-6 py-5 text-[10px] font-black text-slate-300">#{e.id}</td>
                  <td className="px-6 py-5 text-xs font-bold text-slate-800">{new Date(e.date).toLocaleDateString()}</td>
                  <td className="px-6 py-5 text-xs font-bold text-slate-800">{e.project_name || 'N/A'}</td>
                  <td className="px-6 py-5">
                    <div className="text-xs font-bold text-slate-800">{e.paid_to}</div>
                    <div className="text-[10px] text-slate-400">{e.user_name}</div>
                  </td>
                  <td className="px-6 py-5 text-sm font-black text-slate-900 tracking-tight">₹{e.amount.toLocaleString()}</td>
                  <td className="px-6 py-5">
                  ...
                  <td className="px-6 py-5 text-center">
                    <button 
                      ...
                      className="p-2 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white"
                    >
                      <ImageIcon size={16}/>
                    </button>
                  </td>
